...
:::message
以後この本では `$` のプロンプトをホストマシン、`#` のプロンプトをコンテナとして使い分けます。
:::

`--interactive` オプションと `--tty` オプションを付け忘れると、標準入出力が使えないので即時ホストマシンのターミナルに操作が戻ってしまいます。
...
この本では基本的には `--detach` オプションを指定することにしますが、動くことが確認できていないコマンドを試行錯誤しながら組み立てる段階では動作確認がしやすいので外す方が良いでしょう。
状況に合わせて付け外ししてください。
...
この本ではわかりやすくかつランダム性が排除できるため、`--name` オプションは常に指定します。

# コンテナのデフォルト命令を上書きする
オプションではありませんが、`container run` の `[command]` は正しく理解する必要がある重要な引数なので、ここで確認します。
...
`container run` で `[command]` を指定しなかった場合はイメージごとに決まっているデフォルト命令が実行されますが、`[command]` を指定するとデフォルト命令を上書きすることができます。
...
例１の方のコンテナで `bash` を起動するには、【 ２部: コンテナに接続する 】で説明する `container exec` を使います。
...
`--platform=linux/arm64/v8` | Intel or AMD CPU | まず目にしない<br>この本にはない
`--platform=linux/arm64/v8` | ARM CPU         | 意味がない<br>まず目にしない<br>この本にはない
