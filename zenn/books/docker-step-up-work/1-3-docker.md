...
# Docker Engine とは
Docker Engine が 【 １部: 仮想化とは 】 で紹介したコンテナ型仮想化ソフトウェアの部分です。
...
「２つのコンテナを起動し」「それぞれのネットワークを構築し」「コンテナのデータをホストマシンと共有させる」という複雑なコマンドを、Yaml ファイルを書くことで実現できるツールです。
...
# Kubernetes とは
Kubernetes ( 略して k8s とも ) は多数のコンテナを自動的に管理するオーケストレーションソフトウェアでで、`kubectl apply` のような `kubectl` ではじまるコマンドを提供してくれます。
